  ...
  return (
    <Stack sx={{ height: 1 }}>
      ...
      >
        ...
        <Box>
          <Typography variant="subtitle1">Sofa11 Admin</Typography>
      ...
      <Scrollbar sx={{ px: 1.5, pb: 3, flex: '1 1 auto' }}>
        <ListItemButton
          ...
          href="/sofa11/admin"
          selected={pathname === '/sofa11/admin'}
...
function AdminNavGroup({
  ...
  group: (typeof SOFA11_ADMIN_GROUPS)[number];
  pathname: string;
}) {
  const active = pathname.startsWith(`/sofa11/admin/${group.slug}`);
      ...
      <Collapse in={open} unmountOnExit>
        <Stack sx={{ pl: 3.5, borderLeft: (theme) => `dashed 1px ${theme.vars.palette.divider}`, ml: 2.5 }}>
          {group.sections.map((section) => {
            const href = `/sofa11/admin/${group.slug}/${section.slug}`;
            return (
              <ListItemButton
                key={section.slug}
                component={RouterLink}
...
export function Sofa11AdminLayout({ children }: { children: React.ReactNode }) {
...
export function Sofa11AdminHeading({
  title,
  description,
  breadcrumb,
  action = 'Thêm mới',
...
export function Sofa11AdminKpis({
  items,
}: {
...
export function Sofa11AdminTable({
  columns,
  rows,
}: {
  columns: string[];
