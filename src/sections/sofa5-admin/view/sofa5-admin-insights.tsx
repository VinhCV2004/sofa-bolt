...
import { SOFA5_ADMIN_TOOLS, SOFA5_ADMIN_FUNNEL } from '../sofa5-admin-tools';
...
export function Sofa5AdminRevenueChart() {
  const actual = SOFA5_ADMIN_REVENUE_SERIES[0].data;
  const target = SOFA5_ADMIN_REVENUE_SERIES[1].data;
...
export function Sofa5AdminFunnel() {
  const top = SOFA5_ADMIN_FUNNEL[0].value;
  ...
  return (
    <Card sx={{ height: 1 }}>
      ...
      <Stack spacing={2} sx={{ p: 3 }}>
        {SOFA5_ADMIN_FUNNEL.map((step, index) => {
          const ratio = (step.value / top) * 100;
          const previous = SOFA5_ADMIN_FUNNEL[index - 1];
...
export function Sofa5AdminQuickLinks() {
  return (
    <Card sx={{ p: 3 }}>
      ...
      <Grid container spacing={2}>
        {SOFA5_ADMIN_TOOLS.map((tool) => (
          <Grid key={tool.slug} xs={12} sm={6} md={3}>
            <ButtonBase
              component={RouterLink}
              href={`${SOFA5_ADMIN_ROOT}/tools/${tool.slug}`}
              sx={{
                width: 1,
                height: 1,
                p: 2,
