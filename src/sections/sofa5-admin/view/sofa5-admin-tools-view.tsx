...
import {
  ...
  SOFA5_ADMIN_TOOLS,
  SOFA5_NOTIFICATIONS,
  SOFA5_SETTINGS_FIELDS,
  SOFA5_SETTINGS_TOGGLES,
  findSofa5AdminTool,
} from '../sofa5-admin-tools';
import { Sofa5AdminCharts } from './sofa5-admin-charts';
import { Sofa5AdminFunnel, Sofa5AdminRevenueChart } from './sofa5-admin-insights';
import { Sofa5AdminLayout } from './sofa5-admin-layout';
import { SOFA5_ADMIN_ROOT } from '../sofa5-admin-data';
...
export function Sofa5AdminToolsView() {
  const { tool: toolSlug } = useParams();
  const tool = findSofa5AdminTool(toolSlug ?? 'reports');
  ...
  const [settings, setSettings] = useState<SettingsValues>(() =>
    Object.fromEntries(SOFA5_SETTINGS_FIELDS.map((field) => [field.key, field.value]))
  );
  const [toggles, setToggles] = useState<ToggleValues>(() =>
    Object.fromEntries(SOFA5_SETTINGS_TOGGLES.map((item) => [item.key, item.on]))
  );

  const notifications = useMemo(
    () =>
      SOFA5_NOTIFICATIONS.filter((item) => {
  ...
  if (!tool) {
    return <Navigate to={`${SOFA5_ADMIN_ROOT}/tools/reports`} replace />;
  }
    ...
    <Stack spacing={3}>
      <Card>
        ...
        <Grid container spacing={2.5} sx={{ p: 3 }}>
          {SOFA5_SETTINGS_FIELDS.map((field) => (
      ...
      <Card>
        ...
        <Grid container spacing={1} sx={{ p: 2.5 }}>
          {SOFA5_SETTINGS_TOGGLES.map((item) => (
      ...
      <Grid xs={12} md={5}>
        <Card sx={{ p: 3, height: 1 }}>
          <Stack alignItems="center" spacing={1.5} sx={{ textAlign: 'center' }}>
            ...
            <Typography variant="h6">{SOFA5_PROFILE.name}</Typography>
            <Chip label={SOFA5_PROFILE.role} color="warning" variant="soft" />
            <Typography variant="body2" sx={{ color: 'text.secondary' }}>{SOFA5_PROFILE.email}</Typography>
          </Stack>
          <Divider sx={{ my: 3 }} />
          <Stack spacing={1.5}>
            {[
              ['Số điện thoại', SOFA5_PROFILE.phone],
              ['Bộ phận', SOFA5_PROFILE.team],
              ['Tham gia từ', SOFA5_PROFILE.joined],
            ].map(([label, value]) => (
        ...
        <Stack spacing={3}>
          <Card>
            ...
            <Stack direction="row" flexWrap="wrap" useFlexGap gap={1} sx={{ p: 3 }}>
              {SOFA5_PROFILE.permissions.map((permission) => (
          ...
          <Card>
            ...
            <Divider />
            <Stack divider={<Divider />}>
              {SOFA5_PROFILE.sessions.map((session) => (
  ...
  return (
    <>
      <Helmet>
        <title>{`${tool.name} | Quản trị Sofa5`}</title>
      ...
      >
        <Stack spacing={3}>
          <Stack direction="row" spacing={1} sx={{ overflowX: 'auto', pb: 0.5 }}>
            {SOFA5_ADMIN_TOOLS.map((item) => (
              <Button
                key={item.slug}
                href={`${SOFA5_ADMIN_ROOT}/tools/${item.slug}`}
